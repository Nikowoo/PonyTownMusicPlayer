...
function playAudio(url, loop, volume, playbackRate) {
    if (currentAudio) {
...
function openModal() {
    var modalHtml = `
        <div id="pt-custom-music-modal" style="display: none; position: fixed; z-index: 9999; left: 0; top: 0; width: 100%; height: 100%; overflow: auto; background-color: rgba(0, 0, 0, 0.5);">
            <div style="background-color: #fefefe; color: black; margin: 15% auto; padding: 20px; border: 1px solid #888; width: 50%;">
                ...
                <label for="uploadFile">Upload MP3:</label>
                <input type="file" id="uploadFile" accept=".mp3" style="margin-bottom: 10px;">
    ...
    playButton.addEventListener('click', function() {
        ...
        if (!url && !uploadFileInput.files.length) {
            alert("Please enter a valid audio URL or upload an MP3 file.");
            return;
        }
        ...
        if (file) {
            ...
            reader.onload = function(event) {
                ...
                playAudio(fileUrl, loopCheckbox.checked, parseFloat(volumeRange.value), parseFloat(playbackRateRange.value));
            };
            reader.readAsDataURL(file);
        } else {
            playAudio(url, loopCheckbox.checked, parseFloat(volumeRange.value), parseFloat(playbackRateRange.value));
        }
        closeModal();
    });
